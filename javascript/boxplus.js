...
(function ($) {
	...
	*/
	function _loadVideo(anchor) {
		...
		if (/\.(ogg|webM)$/i.test(pathname)) {
			...
			viewerflash.empty().append(_getObjectHTML(dims, attrs, classid, codebase, pluginspage)).removeClass(CLASS_UNAVAILABLE);
		}
	...
	* Uses both the <object /> and the <embed /> HTML elements.
	*/
	function _getObjectHTML(dims, attrs, classid, codebase, pluginspage) {
		return '<object classid="clsid:' + classid + '" codebase="' + codebase + '"' + _getAsAttributeList(dims) + '>' +
			_getAsParameterList(attrs) +
			'<embed pluginspage="' + pluginspage + '"' + _getAsAttributeList(dims) + _getAsAttributeList(attrs) + '/>' +
		'</object>';
	}
